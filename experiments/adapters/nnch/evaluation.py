...
def evaluate(training_params, params, state, logger):
    ...
    eval_results = range_evaluation.range_evaluation(eval_params, state, use_tqdm=False)

    return eval_results


def trace(training_params, params, state, length=10):
    ...
    if training_params.is_autoregressive:
        ...
        G = apply_fn(params, state, next(rng_seq), batch["input"])
